...
function SettingsModal({ user, onClose, resumes, onResumesChange }) {
  ...
  const bookmarkletCode = `javascript:(function(){function m(n){var e=document.querySelector('meta[property="'+n+'"]')||document.querySelector('meta[name="'+n+'"]');return e?e.content:'';}function t(sels){for(var i=0;i<sels.length;i++){var e=document.querySelector(sels[i]);if(e&&e.textContent.trim())return e.textContent.trim();}return '';}var role='',company='';try{var ld=document.querySelectorAll('script[type="application/ld+json"]');for(var i=0;i<ld.length;i++){var data=JSON.parse(ld[i].textContent);var arr=Array.isArray(data)?data:[data];for(var j=0;j<arr.length;j++){var d=arr[j];if(d['@type']==='JobPosting'){role=d.title||role;company=(d.hiringOrganization&&d.hiringOrganization.name)||company;}}}}catch(e){}if(!role)role=t(['h1.job-details-jobs-unified-top-card__job-title','.t-24.job-details-jobs-unified-top-card__job-title','h1.top-card-layout__title','[data-testid="jobsearch-JobInfoHeader-title"]','.jobsearch-JobInfoHeader-title']);if(!company)company=t(['.job-details-jobs-unified-top-card__company-name','.topcard__org-name-link','[data-testid="inlineHeader-companyName"]','.jobsearch-CompanyInfoContainer a']);if(!role){role=m('og:title')||document.title;role=role.replace(/\\s*[|\\-–]\\s*(LinkedIn|Indeed.*|Glassdoor.*)$/i,'').trim();}if(!company)company=m('og:site_name');var link=location.href;var jk=new URLSearchParams(location.search).get('vjk')||new URLSearchParams(location.search).get('jk');if(jk&&location.hostname.indexOf('indeed.')>-1)link='https://'+location.hostname+'/viewjob?jk='+jk;var url='https://job-tracker-tau-eight.vercel.app/?capture=1&role='+encodeURIComponent(role)+'&company='+encodeURIComponent(company)+'&link='+encodeURIComponent(link);window.open(url,'_blank');})();`;
